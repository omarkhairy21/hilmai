...
import { getImageContent } from '../../lib/input-normalization';

export const extractReceiptTool = createTool({
  id: 'extract-receipt',
  description: 'Extract transaction details from receipt image using GPT-4o Vision OCR',
  inputSchema: z.object({
    imageUrl: z.string().describe('URL or file path of the receipt image'),
    telegramChatId: z.number().optional().describe('Telegram chat ID of the user'),
  ...
  outputSchema: z.object({
    ...
    currency: z.string().describe('Currency code (USD, EUR, GBP, AED, SAR, etc.)'),
  ...
  execute: async (input: any) => {
    const { imageUrl, telegramChatId, telegramUsername, firstName, lastName } = input;
    ...
    try {
      console.log('📡 Preparing image for GPT-4o Vision API...');

      // Get image content using utility function with strong typing
